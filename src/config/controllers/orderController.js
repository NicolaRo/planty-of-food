...
const createOrder = async (req, res) => {
    ...
    //1.1.1 Avvio una sessione che conterrà la creazione dinuovi prodotti e nuovi ordini e le gestira
  ...
  try {
    ...
    if (!userId || !Array.isArray(products) || products.length === 0) {
      return res
        .status(400)
        .json({ message: "Dati ordine mancanti o non validi" });
    }
    ...
    res
      .status(500)
      .json({ message: "Errore durante la creazione dell'ordine, riprova" });
